    <?php

    /** 
     * EpollSocket Test Client 
     * By James.Huang <shagoo#gmail.com> 
     **/
    function debug($msg)
    {
        // echo $msg;
        error_log($msg, 3, 'c:/socket.log');
    }

    if (true ||   $argv[1]) {
        // $socket_client = stream_socket_client('tcp://0.0.0.0:2000', $errno, $errstr, 30);
        $socket_client = stream_socket_client('tcp://127.0.0.1:2001', $errno, $errstr, 30);
        // stream_set_blocking($socket_client, 0);
        if (! $socket_client) {
            die("$errstr ($errno)");
        } else {
            $msg = trim($argv[1]);
            
            for ($i = 0; $i < 1 ; $i ++) {
                $res = fwrite($socket_client, "$msg($i)");
//                 usleep(100000);
                debug(fread($socket_client, 1024));
            }
            fclose($socket_client);
        }
    } else {
        
        $phArr = array();
        for ($i = 0; $i < 10; $i ++) {
            $phArr[$i] = popen("D:\\phpStudy\\php\\php-5.4.45-nts\\php " . __FILE__ . " '{$i}:test'", 'r');
        }
        foreach ($phArr as $ph) {
            pclose($ph);
        }
        
        // for ($i = 0; $i < 10; $i++) {
        // system("php ".__FILE__." '{$i}:test'");
        // }
    }  